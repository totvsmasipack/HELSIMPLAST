...
/*/
User Function HCOMR110( cAlias, nReg, nOpcx )
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
		...
		While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM == cNumSC7
			...
			If oReport:Row() > oReport:LineHeight() * 50
				
				oReport:SkipLine()
				oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * (Len(_aObs)*2.15), nPageWidth )
				oReport:SkipLine()
				oReport:PrintText(STR0077,, 050 ) // Observações
				
				For _nPos := 1 To Len(_aObs)
					oReport:SkipLine()
					oReport:PrintText(_aObs[_nPos],, 050 )
				Next _nPos
			...
			AADD(_aObs, 'Item ' + SC7->C7_ITEM + ": " + ALLTRIM(SC7->C7_OBS) )
...
@version 1.0
/*/
User Function R110cObs(aAux1, nTamLinha)	
	Local cVar
	Local nObs := 1
